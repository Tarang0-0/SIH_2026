...
export default function FeaturesPage() {
  ...
  const architectures = [
    {
      step: '01',
      badge: t('features_step1_badge', 'DATA INGESTION'),
      title: t('features_step1_title', 'RTIS & Schedule Integration'),
      desc: t('features_step1_desc', 'Ingests timestamped live train telemetry from the configured authorised provider and combines it with the route timetable catalog.'),
      metric: t('features_step1_metric', 'Provider-backed ingestion'),
      color: 'text-cyan-400',
      border: 'hover:border-cyan-400/40',
    },
    {
      step: '02',
      badge: t('features_step2_badge', 'ML INFERENCE'),
      title: t('features_step2_title', 'Multi-Quantile XGBoost Engine'),
      desc: t('features_step2_desc', 'Computes P10, P50, and P90 arrival quantiles from the deployed feature contract, with live signals added only when provider-backed training data supports them.'),
      metric: t('features_step2_metric', 'Chronological holdout validation'),
      color: 'text-blue-400',
      border: 'hover:border-blue-400/40',
    },
    {
      step: '03',
      badge: t('features_step3_badge', 'EXPLAINABLE AI'),
      title: t('features_step3_title', 'Tree SHAP Causal Attribution'),
      desc: t('features_step3_desc', 'Deconstructs predictions into human-readable feature contributions and reports the model factors available for the current request.'),
      metric: t('features_step3_metric', 'Transparent feature attribution'),
      color: 'text-amber-400',
      border: 'hover:border-amber-400/40',
    },
    {
      step: '04',
      badge: t('features_step4_badge', 'CLIENT TELEMETRY'),
      title: t('features_step4_title', 'Real-Time SSE Gateway'),
      desc: t('features_step4_desc', 'Streams live coordinates, speed, and revised downstream station arrival estimates to client dashboards with zero manual page refreshes.'),
      metric: t('features_step4_metric', 'Provider timestamped updates'),
  ...
  return (
    <div className="min-h-screen bg-[#f7f9fc] dark:bg-transparent text-[#1e293b] dark:text-slate-100 flex flex-col font-sans relative overflow-x-hidden selection:bg-sky-500/20">
      ...
      <section className="relative bg-gradient-to-b from-sky-100/60 to-[#f7f9fc] dark:from-[#0b1c38]/50 dark:via-[#09162e]/40 dark:to-transparent border-b border-sky-200/70 dark:border-sky-900/60 py-16 px-4 sm:px-6 lg:px-8 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-sky-50 dark:bg-sky-950/80 border border-sky-200 dark:border-sky-800/80 text-sky-700 dark:text-sky-300 text-xs font-mono mb-4 shadow-xs">
            ...
            <span>{t('features_hero_badge', 'ENGINEERING ARCHITECTURE')}</span>
          </div>
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
        <div className="text-xs font-mono uppercase text-sky-600 dark:text-sky-400 font-bold tracking-wider mb-2">
          {t('features_pipeline_badge', 'End-to-End Pipeline')}
        </div>
        <h2 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white mb-8">
          {t('features_pipeline_title', 'Data Flow & Predictive Stack')}
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          {architectures.map((arch) => (
            <TiltCard key={arch.step} maxTilt={6} className={`bg-white dark:bg-[#0b1528] p-6 border border-slate-200/90 dark:border-sky-900/60 hover:border-sky-400 dark:hover:border-sky-500 transition-all flex flex-col justify-between rounded-2xl shadow-xs`}>
              ...
              <div className="border-t border-slate-100 dark:border-slate-800/80 pt-3 text-[11px] font-mono text-blue-600 dark:text-blue-400 flex items-center gap-1.5 font-semibold">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
          ...
          <TiltCard maxTilt={5} className="bg-white dark:bg-[#0b1528] p-6 sm:p-7 border border-slate-200/90 dark:border-sky-900/60 rounded-2xl shadow-xs">
            ...
            <div className="bg-slate-50 dark:bg-[#0c1729]/90 border border-slate-200/80 dark:border-slate-800/80 p-4 rounded-xl font-mono text-xs text-slate-700 dark:text-slate-300 space-y-2.5">
              <div className="flex justify-between">
                <span className="text-slate-500 dark:text-slate-400">{t('features_p10_label', 'P10 (Optimistic):')}</span>
                <span className="text-emerald-600 dark:text-emerald-400 font-bold">{t('features_p10_desc', 'Lower-bound ETA interval')}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 dark:text-slate-400">{t('features_p50_label', 'P50 (Expected Median):')}</span>
                <span className="text-sky-700 dark:text-sky-400 font-bold">{t('features_p50_desc', 'Primary ETA estimate')}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 dark:text-slate-400">{t('features_p90_label', 'P90 (Conservative):')}</span>
                <span className="text-amber-600 dark:text-amber-400 font-bold">{t('features_p90_desc', 'Upper-bound ETA interval')}</span>
              </div>
            </div>
          </TiltCard>
          ...
          <TiltCard maxTilt={5} className="bg-white dark:bg-[#0b1528] p-6 sm:p-7 border border-slate-200/90 dark:border-sky-900/60 rounded-2xl shadow-xs">
            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2 flex items-center gap-2">
              ...
              {t('features_deep2_title', 'SHAP Causal Delay Decomposition')}
            </h3>
            <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed mb-4">
              {t('features_deep2_desc', 'Passengers and operators can inspect exact causal drivers. Tree SHAP isolates individual feature contributions for each downstream halt:')}
            </p>
            <div className="bg-slate-50 dark:bg-[#0c1729]/90 border border-slate-200/80 dark:border-slate-800/80 p-4 rounded-xl font-mono text-xs text-slate-600 dark:text-slate-400 space-y-2">
              <div>{t('features_deep2_detail', 'The API returns request-specific feature attribution when the trained model and its explainer are available. Factors include section blockages, weather severity, and priority rake crossings.')}</div>
            </div>
          </TiltCard>
        ...
        <div className="bg-white dark:bg-[#0b1528] p-8 sm:p-10 border border-sky-200 dark:border-sky-900/60 text-center rounded-2xl relative overflow-hidden shadow-xs">
          ...
          <h3 className="text-2xl font-black text-slate-900 dark:text-white mb-2 relative z-10">
            {t('features_cta_title', 'Ready to inspect a running train?')}
          </h3>
