  ...
  return (
    <div className="min-h-screen bg-[#eef7ff] text-slate-900 flex flex-col font-sans relative overflow-x-hidden selection:bg-sky-500/20">
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
        <div className="text-xs font-mono uppercase text-sky-600 font-bold tracking-wider mb-2">End-to-End Pipeline</div>
        <h2 className="text-2xl sm:text-3xl font-black text-slate-900 mb-8">Data Flow & Predictive Stack</h2>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          {architectures.map((arch) => (
            <TiltCard key={arch.step} maxTilt={6} className={`bg-white p-6 border border-slate-200/90 hover:border-sky-400 transition-all flex flex-col justify-between rounded-2xl shadow-xs`}>
              <div>
                ...
                <h3 className="text-base font-bold text-slate-900 mb-2">{arch.title}</h3>
                <p className="text-xs text-slate-600 leading-relaxed mb-4">{arch.desc}</p>
              </div>
              <div className="border-t border-slate-100 pt-3 text-[11px] font-mono text-sky-700 flex items-center gap-1.5 font-semibold">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
          ...
          <TiltCard maxTilt={5} className="bg-white p-6 sm:p-7 border border-slate-200/90 rounded-2xl shadow-xs">
            <h3 className="text-lg font-bold text-slate-900 mb-2 flex items-center gap-2">
              <svg viewBox="0 0 24 24" aria-hidden="true" className="h-5 w-5 text-sky-600" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="m13 2-9 12h7l-1 8 9-12h-7l1-8Z"/></svg>
            ...
            <div className="bg-slate-50 border border-slate-200/80 p-4 rounded-xl font-mono text-xs text-slate-700 space-y-2.5">
              <div className="flex justify-between"><span className="text-slate-500">P10 (Optimistic):</span><span className="text-emerald-600 font-bold">Lower-bound ETA interval</span></div>
              <div className="flex justify-between"><span className="text-slate-500">P50 (Expected Median):</span><span className="text-sky-700 font-bold">Primary ETA estimate</span></div>
              <div className="flex justify-between"><span className="text-slate-500">P90 (Conservative):</span><span className="text-amber-600 font-bold">Upper-bound ETA interval</span></div>
            </div>
          </TiltCard>
          ...
          <TiltCard maxTilt={5} className="bg-white p-6 sm:p-7 border border-slate-200/90 rounded-2xl shadow-xs">
            ...
            <div className="bg-slate-50 border border-slate-200/80 p-4 rounded-xl font-mono text-xs text-slate-700 space-y-2">
              <div className="text-slate-600 leading-relaxed">The API returns request-specific feature attribution when the trained model and its explainer are available. Factors include section blockages, weather severity, and priority rake crossings.</div>
            </div>
          </TiltCard>
        ...
        {/* Action Banner */}
        <div className="bg-white p-8 sm:p-10 border border-sky-200 text-center rounded-2xl relative overflow-hidden shadow-xs">
          <div className="absolute inset-0 bg-gradient-to-r from-sky-500/5 via-blue-500/5 to-transparent pointer-events-none" />
          <h3 className="text-2xl font-black text-slate-900 mb-2 relative z-10">Ready to inspect a running train?</h3>
          <p className="text-xs sm:text-sm text-slate-600 max-w-md mx-auto mb-6 relative z-10 leading-relaxed">
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 bg-gradient-to-r from-sky-500 to-blue-600 hover:from-sky-600 hover:to-blue-700 text-white font-bold text-xs font-mono px-6 py-3 rounded-xl transition-all shadow-sm relative z-10"
          >
