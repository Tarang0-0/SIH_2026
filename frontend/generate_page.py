...
jsx = re.sub(r'<input.*?>', input_replacement, jsx, count=1, flags=re.DOTALL)

# Replace <button ... Track Live ... </button>
...
jsx = re.sub(r'<button[^>]*>.*?Track Live.*?</button>', track_live_btn, jsx, count=1, flags=re.DOTALL)

# Replace "Train 12345 Exp"
jsx = jsx.replace("Train 12345 Exp", "Train {heroData.train_number} - {heroData.train_name}")
# Replace "Approaching Station XYZ"
jsx = jsx.replace("Approaching Station XYZ", "Approaching {heroData.target_halt} | Speed: {heroData.speed} km/h")
